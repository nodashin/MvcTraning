...
namespace MvcOther.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            //入力値が正しい場合にだけ、ユーザーの登録処理を実行
            if (ModelState.IsValid)
            {
                //新規のユーザーを登録
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, MyPage = model.MyPage, Twitter = model.Twitter };
            ...
            return View(model);　//入力エラー時は登録画面を再描画
        }
        ...
        {
            //クエリー情報コードがない場合(=メールのリンク以外からのアクセス)はエラー
            return code == null ? View("Error") : View();
        }

        //入力された情報に基づいてパスワードをリセット
        ...
        {
            //入力値が正しい場合にだけ、パスワードをリセット処理
            if (!ModelState.IsValid)
            {
                return View(model);
            }
            //ユーザーの在否を確認(存在しなければエラー)
            var user = await UserManager.FindByNameAsync(model.Email);
            if (user == null)
            {
                // ユーザーが存在しないことを公開しません。
            ...
            //成功時はリセット成功画面、さもなければ、リセット画面を再描画
            if (result.Succeeded)
            {
                return RedirectToAction("ResetPasswordConfirmation", "Account");
            }
